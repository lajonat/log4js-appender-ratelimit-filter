...
function rateFilter(limits, interval, appender, levels) {
    ...
    checks.TRACE = function () {
        var limit = levelMap[levels.TRACE];
        return checkLimit(limit);
    };
    checks.DEBUG = function () {
        var limit = levelMap[levels.DEBUG];
        checks.TRACE();
        return checkLimit(limit);
    };
    checks.INFO = function () {
        var limit = levelMap[levels.INFO];
        checks.DEBUG();
        return checkLimit(limit);
    };
    checks.WARN = function () {
        var limit = levelMap[levels.WARN];
        checks.INFO();
        return checkLimit(limit);
    };
    checks.ERROR = function () {
        var limit = levelMap[levels.ERROR];
        checks.WARN();
        return checkLimit(limit);
    };
    checks.FATAL = function () {
        var limit = levelMap[levels.FATAL];
        checks.ERROR();
        return checkLimit(limit);
    };
    checks.MARK = function () {
        var limit = levelMap[levels.MARK];
        checks.FATAL();
        return checkLimit(limit);
    };
